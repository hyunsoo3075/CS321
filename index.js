...
function question(ID, answers, content, link, asker) {
    this.ID = ID
...
client.on('message', message => {
	...
	if (message.content.startsWith(`[Question]`)) {
        ...
        var newQuestion = new question((Math.random().toString(36)+'00000000000000000').slice(2, 8), [], msg, message.url, message.author)
     ...
     * gets pushed onto an array of other answers that answer the same question.*/
        ...
        for (var i = 0; i < questions.length; i++) {
            if (questions[i].ID == code) {
                if(questions[i].asker == message.author){                        //edit this and the next four lines out for testing purposes
                    message.channel.send("You cannot answer your own question, your message will be deleted shortly").then(msg => {msg.delete({timeout: 4000})})
...
client.on('messageReactionAdd', (reaction, author) => {
    ...
    /*if(author.username == reaction.message.author.username){ //checks if the author of the answer/question matches the user who reacted to it

        reaction.message.channel.send("You cannot react to your own question/answer.")  =================FIXME==================
...
/*                                                            OBSERVATIONS
 * ---------------------------------------------------------------------------------------------------------------------------------------
 * Someone could repeatedly post questions and answer them for infinite points. //SOLVED//
 * Someone could upvote/downvote their own answer to give themselves 1 point find a way for the bot to unreact the reaction
  */
